...
PENDENTES = Path("data") / "links_pendentes.txt"
...
def gerar_pendentes(candidatos: list[dict], limite: int = 60) -> None:
    """
    Escreve data/links_pendentes.txt com os produtos que MAIS aparecem e
    ainda nao tem link rastreado - a fila de trabalho manual, ja na ordem
    em que da mais retorno gerar.
    """
    tabela = carregar()
    ...
    linhas = [
        "PRODUTOS SEM LINK DE AFILIADO - por ordem de prioridade",
        "",
    ...
    print(f"[links] {len(faltando)} produto(s) sem link - "
